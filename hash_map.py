...
# Assignment: 5
# Student: Elias Kalphat-Losego
# Description: Implement hash table using dynamic array to store the table and
...
class HashMap:
    ...
    def clear(self) -> None:
        ...
        """
        emptyBucket = LinkedList()
        for i in range(self.capacity):
            self.buckets.set_at_index(i, emptyBucket)
        self.size = 0
    ...
    def get(self, key: str) -> object:
        ...
        size = self.capacity
        index = hashVal % size
        if self.buckets.get_at_index(index).contains(key) is None:
            return None
        else:
    ...
    def put(self, key: str, value: object) -> None:
        ...
        oldVal = self.buckets.get_at_index(index).contains(key)
    ...
    def contains_key(self, key: str) -> bool:
        """
        Returns True if the given key is in the hash table, returns
        False otherwise.
        """
        hashVal = self.hash_function(key)
        size = self.capacity
    ...
    def resize_table(self, new_capacity: int) -> None:
        ...
        for i in range(self.capacity):
            ...
            if bucket.length() == 0:
                ...
                for item in bucket:
                    oldVals.insert(item.key, item.value)
    ...
    def get_keys(self) -> DynamicArray:
        ...
        """
        newArray = DynamicArray()
        for bucket in range(self.buckets.length()):
            for item in self.buckets.get_at_index(bucket):
                newArray.append(item.key)
        return newArray
